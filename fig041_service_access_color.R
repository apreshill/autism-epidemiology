...
# saved final PDF
sink_fig <- "./figs/fig041_service_access_colour.pdf"
...
#reshape the fig041 data to looooong
fig041 <- all_3 %>%
...
# initialize figure
fig041 <- ggplot(fig041, aes(x = factor(time), y = estimate))

# adding jittered points
fig041 <- fig041 + geom_jitter(aes(colour = factor(access)), 
                               position=position_jitter(width=.25, height = 0), 
                               alpha = .75,
                               size = 2) 

# formatting axes, plot area, + colors
fig041 <- fig041 + scale_x_discrete(name = "", labels = c("Time 1", "Time 2")) 
fig041 <- fig041 + scale_y_continuous(name = "ASD Cases per 10,000") 
fig041 <- fig041 + coord_cartesian(ylim = c(0,120), xlim = c(.6, 4)) 
fig041 <- fig041 + scale_colour_manual(name = "ASD cases who are:", 
                                     values = wes_palette("FantasticFox")[c(2:3)], 
                                     labels = c("Not accessing services", 
                                                "Accessing services"))  

# adding horizontal line for true prevalence
fig041 <- fig041 + geom_segment(aes(x = .6, xend = 2.5, y = 105, yend = 105), 
                                lty = 3, lwd = .5, colour = "black") 

# creating jagged line segments
fig041 <- fig041 + geom_segment(aes(x = .6, xend = 1.2, y = 30, yend = 30), 
                                lty = 3, lwd = .5, colour = "black") 
fig041 <- fig041 + geom_segment(aes(x = 1.2, xend = 1.8, y = 30, yend = 60), 
                                lty = 3, lwd = .5, colour = "black") 
fig041 <- fig041 + geom_segment(aes(x = 1.8, xend = 2.5, y = 60, yend = 60), 
                                lty = 3, lwd = .5, colour = "black") 

# adding right text
fig041 <- fig041 + annotate("text", 
...
fig041 <- fig041 + annotate("text", x = 2.6, y = 60, 
...
fig041 <- fig041 + theme(axis.line = element_blank())
fig041 <- fig041 + theme(panel.grid.major = element_blank())
fig041 <- fig041 + theme(panel.grid.minor = element_blank())
fig041 <- fig041 + theme(legend.position=c(.7,.2))
fig041 <- fig041 + theme(legend.text = element_text(size = 10))
fig041 <- fig041 + theme(legend.title = element_text(size = 10))
fig041 <- fig041 + theme(legend.background = element_rect(fill="gray90", size=.25, 
                                                          linetype="dotted"))
fig041 <- fig041 + theme(axis.title.y = element_text(size=10))
fig041 <- fig041 + theme(axis.text = element_text(size=10))

ggsave(fig041, file = sink_fig, width = 7, height = 6, dpi = 600)
